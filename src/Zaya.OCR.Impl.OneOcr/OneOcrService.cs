...
public sealed class OneOcrService : IOCRService
{
    ...
    /// </summary>
    public OneOcrService() : this(EmptyLoggingWrapper.Instance)
    {
    ...
    private async Task<IOCRSession> CreateSessionAsync(SettingDescriptorList settingDescriptorList, CancellationToken cancellationToken = default)
    {
        ...
        var wrappedEngine = _loggingWrapper.Wrap<IOneOcrEngine>(engine);
        return _loggingWrapper.Wrap<IOCRSession>(new OneOcrSession(wrappedEngine, minConfidence));
    }
