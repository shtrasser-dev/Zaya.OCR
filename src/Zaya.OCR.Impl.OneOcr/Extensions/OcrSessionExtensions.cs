...
public static class OcrSessionExtensions
{
    ...
    private static IRawImage BitmapToRawImage(Bitmap bitmap)
    {
        ...
        try
        {
            ...
            return new RawImageWrapper(pixels, bitmap.Width, bitmap.Height, stride, Primitives.PixelFormat.Bgra32);
        }
        finally
        {
    ...
    private sealed class RawImageWrapper : IRawImage
    {
        ...
        public Primitives.PixelFormat Format { get; }

        public RawImageWrapper(byte[] pixels, int width, int height, int stride, Primitives.PixelFormat format)
        {
            _pixels = pixels;
            Width = width;
            Height = height;
