...
/// </summary>
internal sealed class LinePreviousMatcher : ILinePreviousMatcher
{
    private readonly ProximityTextLayoutOptions _options;
    private readonly ILineRailSnapper _snapper;

    public LinePreviousMatcher(ProximityTextLayoutOptions options, ILoggingWrapper logging)
    {
        _options = options;
        _snapper = logging.Wrap<ILineRailSnapper>(_ => new LineRailSnapper(options));
    }
    public void Match(List<TextLine> lines, ITextLayoutHistoryService history, bool snap)
    {
    ...
    private static TextLine? FindByMidBody(TextLine line, ITextLayoutHistoryService history)
    {
        var dir = line.Bounds.Direction;
