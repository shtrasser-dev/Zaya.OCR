...
/// <summary>
/// OCR session backed by <see cref="IWindowsMediaOcrEngine"/> (<c>Windows.Media.Ocr</c>).
/// </summary>
public sealed class WindowsMediaOcrSession : IOCRSession
{
    private readonly IWindowsMediaOcrEngine _engine;
    private bool _disposed;

    internal WindowsMediaOcrSession(IWindowsMediaOcrEngine engine)
    {
        _engine = engine;
    }
    ...
    public async Task<IOCRResult> RecognizeAsync(IRawImage image, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ocrResult = await _engine.RecognizeAsync(softwareBitmap, cancellationToken);
        }
